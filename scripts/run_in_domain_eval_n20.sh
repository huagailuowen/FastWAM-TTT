...
OUT="./evaluate_results/dynamic_carrier/20260610_slow100_in_domain_n20_cases000_019"
META="./data/ttt_dynamic_carrier_slow100_observe10_shard00_lerobot/dynamic_carrier_generation_metadata.json"
...
COMMON_ARGS=(
  ...
  "+EVALUATION.max_rollout_videos=5"
...
echo "[$(date -Is)] starting raw in-domain eval"
CUDA_VISIBLE_DEVICES=0 HYDRA_FULL_ERROR=1 .venv/bin/python experiments/dynamic_carrier/eval_dynamic_carrier_single.py \
  ...
  +EVALUATION.run_name=raw_step010000_execution_start \
  +EVALUATION.action_infer_updates_ttt=false \
  "${COMMON_ARGS[@]}" 2>&1 | tee logs/eval_20260610/raw_step010000_execution_start_n20.log

echo "[$(date -Is)] starting stage1 W0 in-domain eval"
CUDA_VISIBLE_DEVICES=0 HYDRA_FULL_ERROR=1 .venv/bin/python experiments/dynamic_carrier/eval_dynamic_carrier_single.py \
  ...
  +EVALUATION.run_name=stage1_step010000_w0_execution_start_no_inner_update \
  +EVALUATION.action_infer_updates_ttt=false \
  ...
  "${COMMON_ARGS[@]}" 2>&1 | tee logs/eval_20260610/stage1_step010000_w0_execution_start_no_inner_update_n20.log

echo "[$(date -Is)] starting TTT step002000 in-domain observe eval"
CUDA_VISIBLE_DEVICES=0 HYDRA_FULL_ERROR=1 .venv/bin/python experiments/dynamic_carrier/eval_dynamic_carrier_single.py \
  ...
  +EVALUATION.run_name=ttt_step002000_backup_observe40_60_execution_start \
  ...
  "${COMMON_ARGS[@]}" 2>&1 | tee logs/eval_20260610/ttt_step002000_backup_observe40_60_execution_start_n20.log

echo "[$(date -Is)] all evals finished"
